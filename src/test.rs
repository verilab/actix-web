...
use crate::data::Data;
use crate::dev::{Body, MessageBody, Payload, SingleThreadServer};
...
///         .to_request();
///
///     let result: Person = test::read_response_json(&app, req).await;
/// }
/// ```
    ...
    B: MessageBody + 'static,
{
    ...
    thread::spawn(move || {
        ...
        let srv = sys.block_on(async {
            let builder = SingleThreadServer::build().workers(1).disable_signals();
            match cfg.stream {
                StreamType::Tcp => match cfg.tp {
                    HttpVer::Http1 => {
                        builder.listen::<_, _, TcpStream>("test", tcp, move || {
