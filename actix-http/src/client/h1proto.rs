...
) -> Result<(ResponseHead, Framed<T, h1::ClientCodec<ActixRuntime>>), SendRequestError>
where
    T: AsyncRead + AsyncWrite + Unpin + 'static,
{
    // create Framed and send request
...
pub(crate) async fn send_body<T, B>(
    body: B,
    mut framed: Pin<&mut Framed<T, h1::ClientCodec<ActixRuntime>>>,
...
pub(crate) struct PlStream<Io> {
    #[pin]
    framed: Option<Framed<Io, h1::ClientPayloadCodec<ActixRuntime>>>,
}

impl<Io: ConnectionLifetime> PlStream<Io> {
    fn new(framed: Framed<Io, h1::ClientCodec<ActixRuntime>>) -> Self {
