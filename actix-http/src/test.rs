...
// a dummy listener which is never used by TestBuffer/TestSeqBuffer.
// TestBuffer/TestSeqBuffer is not generated by any listener type.
...
// dummy impl. In order to reduce trait bound boilerplate for Stream types.
// They are all group into ServiceStream trait.
// Include AsyncRead and AsyncWrite which the real trait bound TestBuffer need.
...
// They are all group into ServiceStream trait.
// Include AsyncRead and AsyncWrite which the real trait bound TestBuffer need.
impl ServiceStream for TestSeqBuffer {
    type Runtime = ActixRuntime;
    type Listener = DummyListener;
    type Item = TcpStream;
    type Error = io::Error;
...
impl IntoStream<TestSeqBuffer> for TcpStream {
    fn into_stream(self, _: CounterGuard) -> TestSeqBuffer {
        unimplemented!()
    }
}
