    ...
    S: Service,
{
    ...
    type InitError = E;
    type Future = TimeoutFuture<S, E>;

    fn new_transform(&self, service: S) -> Self::Future {
        let service = Some(TimeoutService {
            service,
...
impl<S: Service, E> Future for TimeoutFuture<S, E> {
    type Output = Result<TimeoutService<S>, E>;
...
mod tests {
    ...
    impl Service for SleepService {
        ...
        fn call(&self, _: ()) -> Self::Future {
            let fut = actix_rt::time::sleep(self.0);
            Box::pin(async move {
                fut.await;
                Ok(())
            })
